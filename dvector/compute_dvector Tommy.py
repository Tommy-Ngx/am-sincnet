...
model_file = '/content/tommy/AM-MobileNet1D/exp/AM_MobileNet1D_MIT/model_raw_352.pkl'
# Config file of the speaker-id experiment used to generate the model
cfg_file = '/content/tommy/AM-MobileNet1D/cfg/AM_MobileNet1D_MIT.cfg'
te_lst = '/content/tommy/AM-MobileNet1D/data_lists/MIT_test.scp'  # List of the wav files to process
# output dictionary containing the a sentence id as key as the d-vector as value
out_dict_file = 'AM-MobileNet1D/d_vect_mit_1.npy'
data_folder = '/content/tommy/dataset/mit_preprocessing/'
...
with torch.no_grad():			
	...
	for i in range(snt_te):
		...
		if nan_sum>0:
			...
			print(wav_lst_te[i])
			sys.exit(0)
		print('số chiều của d-vector: ',d_vect_out.shape) # saving the d-vector in a numpy dictionary
		dict_key=wav_lst_te[i].split('/')[-2]+'/'+wav_lst_te[i].split('/')[-1]
		d_vect_dict[dict_key]=d_vect_out.cpu().numpy()
		print('dict_key đây: ',dict_key)
    #print('out_dict_key đây: ',out_dict_key)
